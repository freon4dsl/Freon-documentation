...
import { XXunit } from "./internal.js";

/**
 * Class XX is the implementation of the model with the same name in the language definition file.
 * It uses mobx decorators to enable parts of the language environment, e.g. the editor, to react
 * to any changes in the state of its properties.
 */
export class XX extends MobxModelElementImpl implements FreModel {
    /**
     * A convenience method that creates an instance of this class
     * based on the properties defined in 'data'.
     * @param data partial object
     */
    static create(data: Partial<XX>): XX {
        const result = new XX(data.$id);
        if (!!data.name) {
    ...
    readonly $typename: string = "XX"; // holds the metatype in the form of a string
    ...
    units: XXunit[]; // implementation of part 'units'
     ...
     * @param toBeMatched
     */
    public match(toBeMatched: Partial<XX>): boolean {
    ...
    addUnit(newUnit: FreModelUnit): boolean {
        if (!!newUnit) {
            ...
            switch (myMetatype) {
                case "XXunit": {
                    this.units.push(newUnit as XXunit);
                    return true;
                }
            }
        }
    ...
    removeUnit(oldUnit: FreModelUnit): boolean {
        if (!!oldUnit) {
            ...
            switch (myMetatype) {
                case "XXunit": {
                    this.units.splice(this.units.indexOf(oldUnit as XXunit), 1);
                    return true;
                }
            }
        }
    ...
    newUnit(typename: string): FreModelUnit {
        switch (typename) {
            case "XXunit": {
                const unit: XXunit = XXunit.create({});
                this.units.push(unit as XXunit);
                return unit;
            }
        }
        return null;
