...
import { Education } from "../../language/gen/index.js";
import { EducationGrammarStr } from "./EducationGrammar.js";
import { EducationSyntaxAnalyser } from "./EducationSyntaxAnalyser.js";
import { EducationSemanticAnalyser } from "./EducationSemanticAnalyser.js";

/**
 *   Class EducationModelUnitReader is a wrapper for the various parsers of
 *   modelunits.
 */
export class EducationModelUnitReader implements FreReader {
    analyser: EducationSyntaxAnalyser = new EducationSyntaxAnalyser();
    parser: LanguageProcessor = Agl.processorFromString(EducationGrammarStr, this.analyser, null, null);
     ...
     */
    readFromString(sentence: string, metatype: string, model: Education, sourceName?: string): FreModelUnit {
        this.analyser.sourceName = sourceName;
        let startRule: string = "";
        // choose the correct parser
        if (metatype === "Flow") {
            startRule = "Flow";
        } else if (metatype === "SiteGroup") {
            startRule = "SiteGroup";
        } else if (metatype === "Test") {
            startRule = "Test";
        } else if (metatype === "Topic") {
            startRule = "Topic";
        }
            ...
            if (!!model) {
                try {
                    if (model.getUnits().filter((existing) => existing.name === unit.name).length > 0) {
                        ...
                        model.addUnit(unit);
                        const semAnalyser = new EducationSemanticAnalyser();
