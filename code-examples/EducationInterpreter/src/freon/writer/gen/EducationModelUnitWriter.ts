...
export class EducationModelUnitWriter implements FreWriter {
    ...
    public writeNameOnly(modelelement: FreNode): string {
        ...
        if (modelelement instanceof Flow) {
        ...
        } else if (modelelement instanceof SiteGroup) {
            return modelelement.name;
            ...
            return modelelement.name;
        } else if (modelelement instanceof PrimitiveType) {
            return modelelement.name;
    ...
    private unparse(modelelement: FreNode, short: boolean) {
        ...
        switch (modelelement.freLanguageConcept()) {
            ...
            case "Grade":
                ...
                break;
            case "PrimitiveType":
                this.unparsePrimitiveType(modelelement as PrimitiveType, short);
                break;
            case "GradeScore":
            ...
            case "Scenario":
                ...
                break;
            case "TestFlow":
                this.unparseTestFlow(modelelement as TestFlow, short);
                break;
            case "Step":
                this.unparseStep(modelelement as Step, short);
                break;
            case "LastStep":
                this.unparseLastStep(modelelement as LastStep, short);
                break;
            case "Answer":
                this.unparseAnswer(modelelement as Answer, short);
                break;
            case "Page":
            ...
            case "Flow":
                ...
                break;
            case "SiteGroup":
                this.unparseSiteGroup(modelelement as SiteGroup, short);
                break;
            case "Test":
    ...
    private unparseFlow(modelelement: Flow, short: boolean) {
        ...
        this.output[this.currentLine] += `for SiteGroup `;
        this._unparseReference(modelelement.main, short);
    ...
    private unparseFlowRule(modelelement: FlowRule, short: boolean) {
        ...
        if (!short) {
            ...
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `From page `;
    ...
    /**
     * Unparsing of 'SiteGroup' according to projection 'default'.
     */
    private unparseSiteGroup(modelelement: SiteGroup, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `SiteGroup Overview `;
        ...
        if (!short) {
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 4);
            this.output[this.currentLine] += `Topics: `;
            this._unparseReferenceList(modelelement.topics, "", SeparatorType.Separator, true, this.output[this.currentLine].length, short);
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += "";
            ("");
            this.newlineAndIndentation(blockIndent + 4);
            this.output[this.currentLine] += `Flows: `;
            this._unparseReferenceList(modelelement.flows, "", SeparatorType.Separator, true, this.output[this.currentLine].length, short);
        }
    }
    ...
    private unparseQuestionReference(modelelement: QuestionReference, short: boolean) {
        this.output[this.currentLine] += `Answer to `;
        this._unparseReference(modelelement.question, short);
        this.output[this.currentLine] += `is correct `;
    }
    ...
    private unparseNrOfCorrectAnswers(modelelement: NrOfCorrectAnswers, short: boolean) {
        this.output[this.currentLine] += `Number Of Correct Answers `;
    }
    ...
    private unparseTest(modelelement: Test, short: boolean) {
        ...
        if (!short) {
            // do the rest of the lines as well
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Regarding MainTopic: `;
            this._unparseReference(modelelement.main, short);
            this.output[this.currentLine] += `, and flow: `;
    ...
    /**
     * Unparsing of 'TestFlow' according to projection 'default'.
     */
    private unparseTestFlow(modelelement: TestFlow, short: boolean) {
        this._unparseList(
            modelelement.steps,
            "===> ",
            SeparatorType.Separator,
    ...
    /**
     * Unparsing of 'LastStep' according to projection 'default'.
     */
    private unparseLastStep(modelelement: LastStep, short: boolean) {
        this._unparseReference(modelelement.fromPage, short);
    }
    ...
    private unparseTopic(modelelement: Topic, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `SiteGroup: `;
        this._unparseReference(modelelement.main, short);
        ...
        if (!short) {
            ...
            this.output[this.currentLine] += `${modelelement.name} `;
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Topic description: `;
            this.output[this.currentLine] += `"${modelelement.description}" `;
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += "";
            ("");
    ...
    private unparseInDepthMaterial(modelelement: InDepthMaterial, short: boolean) {
        ...
        if (!short) {
            // do the rest of the lines as well
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `InDepthMaterial `;
            this.unparsePage(modelelement, short);
            this.newlineAndIndentation(blockIndent + 4);
            this._unparseList(
                modelelement.content,
    ...
    private unparseQuestion(modelelement: Question, short: boolean) {
        ...
        if (!short) {
            ...
            this.newlineAndIndentation(blockIndent + 4);
            this.output[this.currentLine] += `Correct Answer: `;
            this.unparse(modelelement.correctAnswer, short);
        }
    }
    ...
    /**
     * Unparsing of 'Answer' according to projection 'default'.
     */
    private unparseAnswer(modelelement: Answer, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `Answer `;
        this.output[this.currentLine] += `\{ `;
    ...
    /**
     * The limited concept 'PrimitiveType' is unparsed as its name.
     */
    private unparsePrimitiveType(modelelement: PrimitiveType, short: boolean) {
        if (!!modelelement) {
            this.output[this.currentLine] += modelelement.name + " ";
        }
    }
    ...
    /**
     * The interface 'Type' is not unparsed.
     */
    private unparseType(modelelement: FreNode, short: boolean) {
        throw new Error("Method unparseType should be implemented by the classes that implement it.");
    }
    ...
    private unparsePage_footing(modelelement: Page, short: boolean) {
        ...
        if (!short) {
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Score `;
            this.newlineAndIndentation(blockIndent + 4);
            this._unparseList(
                modelelement.grading,
                "",
                SeparatorType.Separator,
                true,
                this.output[this.currentLine].length,
        ...
        if (!!modelelement) {
            ...
            if (!!type) {
                if (type instanceof Grade) {
                    ...
                    this.unparsePrimitiveType(type, short);
                } else {
