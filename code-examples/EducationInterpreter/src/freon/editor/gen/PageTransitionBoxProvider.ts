// Generated by the Freon Language Generator.
import {
    Box,
    BoxUtil,
    FreBoxProvider,
    FreProjectionHandler,
    FreNodeReference,
    LimitedDisplay,
    TableRowBox,
    TableUtil,
    BoxFactory,
} from "@freon4dsl/core";

import { PageTransition, Grade, Page } from "../../language/gen/index.js";

import { EducationEnvironment } from "../../config/gen/EducationEnvironment.js";

/**
 * This class implements the box provider for a single node of type PageTransition.
 * The box provider is able to create the (tree of) boxes for the node, based
 * on the projections that are currently selected by the user.
 * The top of the tree of boxes is always a box of type ElementBox, which is
 * a box that will never be rendered itself, only its content will. Thus, we
 * have a stable entry in the complete box tree for every FreNode node.
 */
export class PageTransitionBoxProvider extends FreBoxProvider {
    constructor(mainHandler: FreProjectionHandler) {
        super(mainHandler);
        this.knownBoxProjections = ["default"];
        this.knownTableProjections = ["tableRowFor_rules_as_table"];
        this.conceptName = "PageTransition";
    }

    protected getContent(projectionName: string): Box {
        // console.log("GET CONTENT " + this._node?.freId() + ' ' +  this._node?.freLanguageConcept() + ' ' + projectionName);
        // see if we need to use a custom projection
        if (!this.knownBoxProjections.includes(projectionName) && !this.knownTableProjections.includes(projectionName)) {
            const BOX: Box = this.mainHandler.executeCustomProjection(this._node, projectionName);
            if (!!BOX) {
                // found one, so return it
                return BOX;
            }
        } else {
            // select the box to return based on the projectionName
            if (projectionName === "default") {
                return this.getDefault();
            } else if (projectionName === "tableRowFor_rules_as_table") {
                return this.getTableRowFor_rules_as_table();
            }
        }
        // in all other cases, return the default
        return this.getDefault();
    }

    private getTableRowFor_rules_as_table(): TableRowBox {
        const cells: Box[] = [];
        cells.push(
            BoxUtil.limitedBox(
                this._node as PageTransition,
                "condition",
                (selected: string) => {
                    (this._node as PageTransition).condition = FreNodeReference.create<Grade>(selected, "Grade");
                },
                LimitedDisplay.SELECT,
                EducationEnvironment.getInstance().scoper,
            ),
        );
        cells.push(
            BoxUtil.referenceBox(
                this._node as PageTransition,
                "toPage",
                (selected: string) => {
                    (this._node as PageTransition).toPage = FreNodeReference.create<Page>(selected, "Page");
                },
                EducationEnvironment.getInstance().scoper,
            ),
        );
        return TableUtil.rowBox(
            this._node,
            this._node.freOwnerDescriptor().propertyName,
            "PageTransition",
            cells,
            this._node.freOwnerDescriptor().propertyIndex,
            true,
        );
    }

    private getDefault(): Box {
        return BoxFactory.horizontalLayout(
            this._node as PageTransition,
            "PageTransition-hlist-line-0",
            "",
            [
                BoxUtil.labelBox(this._node as PageTransition, "when", "top-1-line-0-item-0"),
                BoxUtil.limitedBox(
                    this._node as PageTransition,
                    "condition",
                    (selected: string) => {
                        (this._node as PageTransition).condition = FreNodeReference.create<Grade>(selected, "Grade");
                    },
                    LimitedDisplay.SELECT,
                    EducationEnvironment.getInstance().scoper,
                ),
                BoxUtil.labelBox(this._node as PageTransition, "goto page", "top-1-line-0-item-2"),
                BoxUtil.referenceBox(
                    this._node as PageTransition,
                    "toPage",
                    (selected: string) => {
                        (this._node as PageTransition).toPage = FreNodeReference.create<Page>(selected, "Page");
                    },
                    EducationEnvironment.getInstance().scoper,
                ),
            ],
            { selectable: false },
        );
    }
}
