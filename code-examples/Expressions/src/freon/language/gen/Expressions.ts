...
import { ExpressionUnit } from "./internal.js";

import { makeObservable, action, runInAction } from "mobx";

/**
 * Class Expressions is the implementation of the model with the same name in the language definition file.
 * It uses mobx decorators to enable parts of the language environment, e.g. the editor, to react
 * to any changes in the state of its properties.
 */
export class Expressions extends MobxModelElementImpl implements FreModel {
    /**
     * A convenience method that creates an instance of this class
     * based on the properties defined in 'data'.
     * @param data partial object
     */
    static create(data: Partial<Expressions>): Expressions {
        const result = new Expressions(data.$id);
        if (!!data.name) {
    ...
    readonly $typename: string = "Expressions"; // holds the metatype in the form of a string
    ...
    units: ExpressionUnit[]; // implementation of part 'units'
     ...
     * @param toBeMatched
     */
    public match(toBeMatched: Partial<Expressions>): boolean {
    ...
    replaceUnit(oldUnit: FreModelUnit, newUnit: FreModelUnit): boolean {
        ...
        if (oldUnit.freLanguageConcept() === "ExpressionUnit" && oldUnit.freOwnerDescriptor().propertyName === "units") {
            AST.changeNamed("removeUnit", () => {
                const index = this.units.indexOf(oldUnit as ExpressionUnit);
                this.units.splice(index, 1, newUnit as ExpressionUnit);
            });
        } else {
            return false;
        }
            ...
            switch (myMetatype) {
                case "ExpressionUnit": {
                    AST.changeNamed("addUnit", () => {
                        this.units.push(newUnit as ExpressionUnit);
                    });
                    return true;
                }
            }
    ...
    removeUnit(oldUnit: FreModelUnit): boolean {
        if (!!oldUnit) {
            ...
            switch (myMetatype) {
                case "ExpressionUnit": {
                    AST.changeNamed("removeUnit", () => {
                        this.units.splice(this.units.indexOf(oldUnit as ExpressionUnit), 1);
                    });
                    return true;
                }
            }
    ...
    newUnit(typename: string): FreModelUnit {
        switch (typename) {
            case "ExpressionUnit": {
                ...
                runInAction(() => {
                    unit = ExpressionUnit.create({});
                });
                AST.changeNamed("newUnit", () => {
                    this.units.push(unit as ExpressionUnit);
                });
                return unit;
            }
        }
