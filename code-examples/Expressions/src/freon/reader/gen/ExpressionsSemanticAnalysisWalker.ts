...
import { ParameterRef, FunctionCallExpression, Parameter, Function } from "../../language/gen/index.js";
import { ExpressionsWorker, ExpressionsDefaultWorker } from "../../utils/gen/index.js";
import { FreNamedNode, FreLanguage, FreLanguageEnvironment, FreNodeReference, FreNode } from "@freon4dsl/core";

export class ExpressionsSemanticAnalysisWalker extends ExpressionsDefaultWorker implements ExpressionsWorker {
    ...
    public execBeforeFunctionCallExpression(modelelement: FunctionCallExpression): boolean {
        let referredElem: FreNodeReference<FreNamedNode>;
        referredElem = modelelement.calledFunction;
        if (!!modelelement.calledFunction && modelelement.calledFunction.referred === null) {
            // cannot find a 'calledFunction' with this name
            this.findReplacement(modelelement, referredElem);
        }
        return false;
    }
    ...
    private findReplacement(modelelement: FreNode, referredElem: FreNodeReference<FreNamedNode>) {
        ...
        if (possibles.length > 0) {
            ...
            for (const elem of possibles) {
                ...
                if (FreLanguage.getInstance().metaConformsToType(elem, "Parameter")) {
                    replacement = ParameterRef.create({ parameter: FreNodeReference.create<Parameter>(referredElem.name, metatype) });
                } else if (FreLanguage.getInstance().metaConformsToType(elem, "Function")) {
                    replacement = FunctionCallExpression.create({
                        calledFunction: FreNodeReference.create<Function>(referredElem.name, metatype),
                    });
                } else {
                    throw new Error(
