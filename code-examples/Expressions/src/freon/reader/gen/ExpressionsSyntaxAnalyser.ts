...
import { ExpressionUnitSyntaxAnalyserPart, ExpressionsCommonSyntaxAnalyserPart } from "./index.js";

/**
 *   Class ExpressionsSyntaxAnalyser is the main syntax analyser.
 ...
 *
 */
export class ExpressionsSyntaxAnalyser implements SyntaxAnalyser {
    sourceName: string = "";
    locationMap: any;
    private _unit_ExpressionUnit_analyser: ExpressionUnitSyntaxAnalyserPart = new ExpressionUnitSyntaxAnalyserPart(this);
    private _unit_common_analyser: ExpressionsCommonSyntaxAnalyserPart = new ExpressionsCommonSyntaxAnalyserPart(this);
    ...
    public transformSharedPackedParseTreeNode(node: SPPTNode): any {
        if (!!node) {
            try {
                ...
                if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in ExpressionsSyntaxAnalyser")) {
                    throw e;
                } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("ExpressionUnit" === brName) {
            return this._unit_ExpressionUnit_analyser.transformExpressionUnit(branch);
        } else if ("Function" === brName) {
            return this._unit_ExpressionUnit_analyser.transformFunction(branch);
        } else if ("NumberLiteralExpression" === brName) {
            return this._unit_ExpressionUnit_analyser.transformNumberLiteralExpression(branch);
        } else if ("ParameterRef" === brName) {
            return this._unit_ExpressionUnit_analyser.transformParameterRef(branch);
        } else if ("FunctionCallExpression" === brName) {
            return this._unit_ExpressionUnit_analyser.transformFunctionCallExpression(branch);
        } else if ("Parameter" === brName) {
            return this._unit_ExpressionUnit_analyser.transformParameter(branch);
        } else if ("Expression" === brName) {
            return this._unit_ExpressionUnit_analyser.transformExpression(branch);
        } else if ("__fre_binary_Expression" === brName) {
            return this._unit_ExpressionUnit_analyser.transform__fre_binary_Expression(branch);
        } else if ("Type" === brName) {
            return this._unit_ExpressionUnit_analyser.transformType(branch);
        } else if ("__fre_reference" === brName) {
            return this.transform__fre_reference(branch);
        } else {
            throw new Error(`Error in ExpressionsSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }
