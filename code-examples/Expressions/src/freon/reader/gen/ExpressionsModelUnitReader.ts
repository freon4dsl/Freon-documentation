...
import { Expressions } from "../../language/gen/index.js";
import { ExpressionsGrammarStr } from "./ExpressionsGrammar.js";
import { ExpressionsSyntaxAnalyser } from "./ExpressionsSyntaxAnalyser.js";
import { ExpressionsSemanticAnalyser } from "./ExpressionsSemanticAnalyser.js";

/**
 *   Class ExpressionsModelUnitReader is a wrapper for the various parsers of
 *   modelunits.
 */
export class ExpressionsModelUnitReader implements FreReader {
    analyser: ExpressionsSyntaxAnalyser = new ExpressionsSyntaxAnalyser();
    parser: LanguageProcessor = Agl.processorFromString(ExpressionsGrammarStr, this.analyser, null, null);
     ...
     */
    readFromString(sentence: string, metatype: string, model: Expressions, sourceName?: string): FreModelUnit {
        this.analyser.sourceName = sourceName;
        let startRule: string = "";
        // choose the correct parser
        if (metatype === "ExpressionUnit") {
            startRule = "ExpressionUnit";
        }
            ...
            if (!!model) {
                try {
                    if (model.getUnits().filter((existing) => existing.name === unit.name).length > 0) {
                        ...
                        model.addUnit(unit);
                        const semAnalyser = new ExpressionsSemanticAnalyser();
