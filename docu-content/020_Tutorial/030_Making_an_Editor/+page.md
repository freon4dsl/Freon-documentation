---
title: Making an Editor for Your DSL
description: Create editor projections in Freon—including defaults, list layouts, inherited projections, and triggers—to make your DSL readable and easy to edit.
tags: Freon, editor, projections, projectional editing, DSL, Topic, Page, Theory, Video, WorkSheet, ExamplePage, InDepthMaterial, Question, Line, Fraction, SimpleNumber, lists, separator, initiator, terminator, triggers, inheritance, Education language, tutorial
---
...
No, of course we are not done. Just kidding—and showing you that Freon will add a default [projection](/Documentation/Defining_an_Editor/Projections) for any concept that is not
...
<Figure
imageName={'tutorial/Tutorial-lesson2-screenshot1.png'}
...
A complete editor definition is built from a number of projections. To define a single projection
...
- A line with the fixed
  ...
  that is projected. For clarity, we will use this prefix throughout the tutorial.
- A line with the fixed string `Topic:` followed by the name of the unit.
- The third line is similar, but projects the `description` property of the model unit. Next, we add an empty line
...
Above, we projected the pages as a vertical list. There are many options to project lists. Not all lists need to be
...
definition above: `terminator[== END OF LINE]`. It means that
after every element of the list the string `== END OF LINE` is projected.
...
the character or string before each element.
...
Remember that we defined the concept `Page` to be abstract, and there were several concepts that inherit from `Page`? Of course,
...
that we include the default projection of _Page_ right there.
...
The last thing to learn in this lesson is how we can make editing easier for the user. Remember that we needed a concept for numeric fractions?
It’s called `Fraction`, and it is formed by combining two numbers: a numerator and a denominator. This is how the concept is defined in the `.ast` file.
...
In Freon, the user must first choose the `Fraction` option from a dropdown menu before they can enter any numbers. To avoid
...
For the `Fraction` concept we define the trigger to be a forward slash. If the user wants to add a correct answer to a question,
where the possibilities are either a `SimpleNumber` or a `Fraction`, entering `/` will produce a `Fraction` instance. So, the
...
the next part of the tutorial, where we’ll have more fun with defining projections.
