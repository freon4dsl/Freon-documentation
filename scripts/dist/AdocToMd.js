...
class AdocToMd {
  ...
  transformFile(filepath) {
    let oldContents = fs.readFileSync(path.join(previousSiteFolder, filepath), {
      encoding: "utf8",
    });
    ...
    contents = contents.replace(
      /:page-has\*children: [_a-zA-Z0-9 -\.\/]*/g,
      ""
    );
    ...
    contents = contents.replace(
      /\/three-levels/g,
      "/050_Three_Levels_of_Customization"
    );
    contents = contents.replace(
      /\/expressions/g,
      "/010_Projectional_Editing#expressions"
    );
    let newFilepath = path.join(
      path.dirname(filepath),
      path.basename(filepath, ".adoc").concat(".md")
    );
